...
{semProducerBuffer1}
\lstinputlisting[firstline=3,lastline=36]{Lst/SemBoundedBuffer.java}
\end{myfig}

\begin{myfig}{btp}{モニタと同等なキューをセマフォで実現（Java版，後半）}
{semProducerBuffer2}
\lstinputlisting[firstline=37,firstnumber=last]{Lst/SemBoundedBuffer.java}
\end{myfig}
...
（\figref{semProducerBuffer1}，\figref{semProducerBuffer2}）を
実際に実行しなさい．
...
\item {\tt MonBoundedBuffer}（\figref{javaMonProducerConsumer}）を
実際に実行しなさい．
メインルーチンを含むソースプログラムは以下から入手できる．\\
