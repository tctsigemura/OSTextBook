...
\myincludegraphics{semaphore.png}{scale=0.4}
\end{myfig}

セマフォ型の変数は内部にカウンタ\footnote{カウンタの値は0以上の整数値である．}を持ち，
また，プロセスの待ち行列を作ることができる．
...
\begin{description}
\item[P操作（P(S)）] %クリティカルセクションのエントリーセクション等で使用できる．
...
\subsubsection{複数生産者と複数消費者の問題（Producer-Consumer problem）}
前の問題で，
...
TacOSのセマフォは\figref{sem}に示す構造体である\footnote{図は
...
\myincludegraphics{tacosSemaphore-crop.pdf}{scale=0.6}
\end{myfig}
...
\subsection{セマフォ割当}
\figref{tacosNewSem}にTacOSカーネル内のセマフォ割当と解放ルーチンを示す\footnote{図は
...
\subsection{P操作ルーチン}
\figref{tacosSemP}にTacOSのP操作ルーチンを示す\footnote{図は
\url{https://github.com/tctsigemura/TacOS/blob/master/os/kernel/kernel.cmm}
の一部である．}．
...
\subsection{V操作ルーチン}
\figref{tacosSemV}にTacOSのV操作ルーチンを示す\footnote{図は
\url{https://github.com/tctsigemura/TacOS/blob/master/os/kernel/kernel.cmm}
の一部である．}．
...
\figref{tacosSetPri}に示す\footnote{図は
