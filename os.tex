%------------------------------------------------------------------------------
%
% OS 教科書
%
%------------------------------------------------------------------------------
%\documentclass[a4paper,11pt,twocolumn,dvipdfmx]{jsbook}   % platex の場合
\documentclass[a4paper,11pt,twocolumn]{ltjsbook}     % lualatex の場合
\usepackage{mySty}
%------------------------------------------------------------------------------
% はじまり
\begin{document}
\frontmatter
%------------------------------------------------------------------------------
% 表紙
\title{オペレーティングシステム\\Ver. 0.0.0}
\author{徳山工業高等専門学校\\情報電子工学科}
\date{}
\maketitle

%------------------------------------------------------------------------------
% 著作権表示
\thispagestyle{empty}
\onecolumn
~
\vfill
\begin{flushleft}
Copyright \copyright ~~ 2017 by \\
Dept. of Computer Science and Electronic Engineering, \\
Tokuyama College of Technology, JAPAN
\end{flushleft}

\vspace{0.8cm}
本ドキュメントはCC-BY-SA 4.0 ライセンスによって許諾されています。

本ドキュメントは
CC-BY-SA 3.0 de ライセンス，
CC-BY-SA 4.0 ライセンス
で許諾された著作物を含みます．

（CC-BY-SA 3.0 de ライセンス全文は
\url{https://creativecommons.org/licenses/by-sa/3.0/de/}で，
CC-BY-SA 4.0 ライセンス全文は
\url{https://creativecommons.org/licenses/by-sa/4.0/deed.ja}で確認できます．）

%------------------------------------------------------------------------------
% 目次
\setcounter{tocdepth}{2}
\tableofcontents

% 本文
\mainmatter

\part{OSとは}
\include{chap1} % オペレーティングシステムとは
\include{chap2} % 前提知識

\part{CPU管理}
\include{chap3} % CPUの仮想化
\include{chap4} % スケジューリング
\include{chap5} % プロセス同期
\include{chap6} % プロセス間通信
\include{chap7} % モニタ

\part{メモリ管理}
\include{chap8} % 主記憶
\include{chap9} % メモリ割付け
\include{chapA} % メモリ割付けプログラムの例
\include{chapB} % セグメンテーション
\include{chapC} % ページング
\include{chapD} % 仮想記憶

\part{ファイル管理}
\setcounter{chapter}{13}
\setcounter{page}{177}               % 途中からコンパイルしたいので
\include{chapE} % 二次記憶装置
\setcounter{chapter}{14}
\setcounter{page}{189}               % 途中からコンパイルしたいので
\include{chapF} % ファイルシステムの概念
\setcounter{chapter}{15}
\setcounter{page}{201}               % 途中からコンパイルしたいので
\include{chapG} % FATファイルシステム

\include{bibliography} % 参考文献
\appendix
\include{appA}  % 付録A
\include{appB}  % 付録B

%------------------------------------------------------------------------------
% 発行元
\backmatter
\pagestyle{empty}
\onecolumn
~
\vfill\vfill\vfill
\begin{center}
\fbox{\parbox{10cm}{ \vspace{0.3cm}
\large{\bf{オペレーティングシステム Ver. 0.0.0}} \\
\\
　発行年月　２０１７年１０月　Ver.0.0.0 \\
　発　　行　独立行政法人国立高等専門学校機構 \\
　　　　　　徳山工業高等専門学校 \\
　　　　　　情報電子工学科　重村哲至 \\
　　　　　　〒745-8585 山口県周南市学園台 \\
　　　　　　sigemura@tokuyama.ac.jp \\
}}
\end{center}
\vfill
\end{document}
